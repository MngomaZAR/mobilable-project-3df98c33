...
-- RLS (use DROP + CREATE because Postgres has no CREATE POLICY IF NOT EXISTS)
DROP POLICY IF EXISTS "video_calls_select_participants" ON public.video_call_sessions;
CREATE POLICY "video_calls_select_participants" ON public.video_call_sessions
  FOR SELECT USING (auth.uid() = creator_id OR auth.uid() = viewer_id);

DROP POLICY IF EXISTS "video_calls_insert_viewer" ON public.video_call_sessions;
CREATE POLICY "video_calls_insert_viewer" ON public.video_call_sessions
  FOR INSERT WITH CHECK (auth.uid() = viewer_id);

DROP POLICY IF EXISTS "video_calls_update_participants" ON public.video_call_sessions;
CREATE POLICY "video_calls_update_participants" ON public.video_call_sessions
  FOR UPDATE USING (auth.uid() = creator_id OR auth.uid() = viewer_id);
